@if(session('findmsg'))
    <script>alert('{{session('findmsg')}}');</script>
@endif
@if(session('editmessage'))
    <script>alert('{{session('editmessage')}}');</script>
@endif
@if(session('deletemsg'))
    <script>alert('{{session('deletemsg')}}');</script>
@endif
@if(session('requestmsg'))
    <script>alert('{{session('requestmsg')}}');</script>
@endif
<!DOCTYPE html>
<html lang="zxx">
...
<section class="gallery-section spad">
    <div class="container">
                        ...
                        <div class="gi-hover-warp">
                            <div class="gi-hover">
                                ...
                                @if(Auth::user()->type == '1')
                                    ...
                                    <a href="/deletesupplement/{{$supplement->id}}"><i class="fa fa-trash"></i></a>
                                @else
                                    <a href="{{$supplement->image}}" class="image-popup"><i
                                            class="fa fa-search-plus"></i></a>

                                    <a href="/requestsupplement/{{$supplement->id}}"><i class="fa fa-plus"></i></a>
                                @endif
                            </div>
                        </div>
                    </div>
        ...
        <a href="/addsupplement" class="primary-btn f-btn">Add supplement</a>
    </div>
</section>
<!-- Feature Section End -->
